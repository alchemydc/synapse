...
export function injectSourceLinks(md: string): string {
  ...
  md = md.replace(/\[Forum\s+([^\]]+)\](?:\s+(disc-topic-(\d+)))?/g, (_m: string, label: string, discToken?: string, topicIdStr?: string) => {
    try {
      ...
      const topicMeta = lookupDiscourseTopicByLabel(label);
      ...
      const catMeta = lookupDiscourseCategoryByLabel(label);
