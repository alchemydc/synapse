...
async function fetchJSON(path: string) {
  if (!BASE) throw new Error("DISCOURSE_BASE_URL not set");
  const url = `${BASE}${path}`;
  const headers = {
    "Api-Key": API_KEY || "",
...
async function main() {
  ...
  const topics =
    (latestResp.json && latestResp.json.topic_list && latestResp.json.topic_list.topics) ||
  ...
  if (!Array.isArray(topics) || topics.length === 0) {
    ...
    for (const t of sample) {
      ...
      console.log(
        `  - id=${t.id} posts=${t.posts_count ?? t.post_count ?? "?"} created_at=${t.created_at ?? t.last_posted_at ?? "?"} title="${t.title}"`
      );
    }
  }
